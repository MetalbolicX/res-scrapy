...
let querySelector = (ops: operations, doc: document, selector: string) =>
  ops.querySelector(doc, selector)
...
let parseDocumentSafely = (ops: operations, html: string): result<document, AppError.appError> => {
  try {
    Ok(parse(ops, html))
  } catch {
