 ...
 */
const padToTwoDigits = (value: number | string): string =>
 ...
 */
const padToFourDigits = (value: number | string): string =>
 ...
 */
const parseTimeZoneOffsetMinutes = (timeZoneName?: string): number => {
 ...
 */
const getUtcDateComponents = (date: Date): DateComponents => ({
 ...
 */
const getTimeZoneDateComponents = (
  date: Date,
  timeZone: string,
): DateComponents => {
 ...
 */
const getDateComponents = (date: Date, timeZone?: string): DateComponents =>
...
const formatTimeZoneOffset = (offsetMinutes: number): string => {
  ...
  return `${sign}${padToTwoDigits(Math.floor(absoluteOffset / 60))}:${padToTwoDigits(
    absoluteOffset % 60,
  )}`;
};
