...
let _ = testAsync("UrlOutputWriter - appendNdjsonToFile returns Ok(()) on success", planned => {
  ...
  appendNdjsonToFile(~appendFile=appendFileMock, ~err=errMock, ~stringifyJson, ~path="out.ndjson", ~json)
  ->Promise.then(result => {
    isResultOk(result)
...
let _ = testAsync("UrlOutputWriter - appendNdjsonToFile returns Error and warns on appendFile failure", planned => {
  ...
  ->Promise.then(result => {
    ...
    isTextEqualTo(`Warning: Failed to append to output file "out.ndjson": Disk full`, errReceived.contents)
    planned(~planned=2, ())
    Promise.resolve()
  })
  ->ignore
...
let _ = testAsync("UrlOutputWriter - appendNdjsonToFile wraps a bare JSON value as a single-row NDJSON line", planned => {
  let fileReceived = ref(("", ""))
  let appendFileMock = (path, text) => {
    fileReceived := (path, text)
    Promise.resolve()
  ...
  appendNdjsonToFile(~appendFile=appendFileMock, ~err=_ => (), ~stringifyJson, ~path="bare.ndjson", ~json)
  ->Promise.then(result => {
    isResultOk(result)
