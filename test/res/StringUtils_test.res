...
test("StringUtils.extractPattern returns first match for simple literal", () => {
  isOptionEqualTo(
    Some("world"),
    StringUtils.extractPattern("hello world", "world"),
    ~eq=(a, b) => a == b,
  )
})
...
test("StringUtils.extractPattern supports character classes", () => {
  isOptionEqualTo(
    Some("ABC123"),
    StringUtils.extractPattern("id=ABC123 end", "[A-Z0-9]+"),
    ~eq=(a, b) => a == b,
  )
})
...
test("StringUtils.extractPattern returns None for zero-width match", () => {
  ...
  isOptionEqualTo(
    None,
    StringUtils.extractPattern("abc", "x*"),
    ~eq=(a, b) => a == b,
  )
})
...
test("StringUtils.extractPattern honors anchors", () => {
  isOptionEqualTo(
    Some("foo"),
    StringUtils.extractPattern("foobar", "^foo"),
    ~eq=(a, b) => a == b,
  )
  isOptionEqualTo(
    None,
    StringUtils.extractPattern("barfoo", "^foo"),
    ~eq=(a, b) => a == b,
  )
})
...
test("StringUtils.extractPattern respects case sensitivity by default", () => {
  isOptionEqualTo(
    None,
    StringUtils.extractPattern("HELLO", "hello"),
    ~eq=(a, b) => a == b,
  )
  isOptionEqualTo(
    Some("HELLO"),
...
test("StringUtils.extractPattern rejects patterns with lookaheads", () => {
  isOptionEqualTo(
    None,
    StringUtils.extractPattern("abc123", "(?=[0-9]+)abc"),
    ~eq=(a, b) => a == b,
  )
})
...
test("StringUtils.extractPattern rejects patterns with backreferences", () => {
  isOptionEqualTo(
    None,
    StringUtils.extractPattern("aa", "(a)\\1"),
    ~eq=(a, b) => a == b,
  )
})
...
test("StringUtils.extractPattern rejects overly large quantifiers", () => {
  isOptionEqualTo(
    None,
    StringUtils.extractPattern("aaaa", "a{10000}"),
    ~eq=(a, b) => a == b,
  )
})
