...
test("fixtures: attributes case", () => {
  let html =
    ...
    "<a class='link' href='/foo' title='Foo Link' data-track='abc'>Foo</a>" ++ "<a class='all-attrs' href='/bar' title='Bar Link' rel='nofollow'>Bar</a>"
  let schema =
    "{\"fields\":{" ++
...
test("fixtures: dates case", () => {
  let html = "<time class='iso' datetime='2024-06-01'>June 1, 2024</time><span class='long'>June 1, 2024</span><span class='us'>06/01/2024</span><span class='euro'>01-06-2024</span>"
  let schema =
    "{\"fields\":{" ++
...
test("fixtures: inner_outer case", () => {
  let html = "<div class='card' id='card-1'><h2 class='title'>Card One</h2><div class='body'><p>First <strong>bold</strong></p><script>bad()</script><style>.x{}</style></div></div>"
  let schema =
    "{\"fields\":{" ++
...
test("fixtures: other case", () => {
  let html = "<div class='product'><span class='price'>$1,234.56</span><span class='discount'>25% OFF</span><span class='feat'>fast</span><span class='feat'>cheap</span><span class='feat'>cheap</span><span class='available'>In Stock</span><script type='application/ld+json'>{\"offers\":{\"price\":\"19.99\"}}</script></div>"
  let schema =
    "{\"fields\":{" ++
...
test("fixtures: row mode with default fallback on missing required", () => {
  let html =
    "<div class='row'><span class='name'>A</span><span class='price'>10</span></div>" ++ "<div class='row'><span class='name'>B</span></div>"
  let schema =
    "{\"fields\":{" ++
    "\"name\":{\"selector\":\".name\",\"type\":\"text\"}," ++
    "\"price\":{\"selector\":\".price\",\"type\":\"number\",\"required\":true,\"default\":0}" ++ "},\"config\":{\"rowSelector\":\".row\",\"ignoreErrors\":true}}"
...
test("fixtures: zip mode aggregate + scalar behavior", () => {
  let html = "<h3>A</h3><h3>B</h3><span class='tag'>x</span><span class='tag'>y</span><span class='tag'>y</span>"
  let schema =
    "{\"fields\":{" ++
