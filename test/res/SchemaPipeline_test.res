...
test("two-field pipeline: name (TextExtractor) + count (TextExtractor) produces correct merged output", () => {
...
test("two-field pipeline: TextExtractor + TextExtractor with different selectors produces distinct values", () => {
  let html = `<html><body>
...
test("three-field pipeline with default merging: title + price (AttributeExtractor) + description (TextExtractor)", () => {
  let html = `<html><body>
    <h1>Shoes</h1>
...
test("three-field pipeline: all TextExtractor fields with different selectors produces distinct values", () => {
  let html = `<html><body>
    <article>
...
test("multi-field pipeline: missing selector on one field still produces output for found fields", () => {
  let html = `<html><body>
